...
export const createBarChartBlock = createReactBlockSpec(
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      const updateProps = useCallback((next: Partial<{
        grid: boolean;
        title: string;
        showTitle: boolean;
        legend: boolean;
      ...
      const handleMetricChange = useCallback((metric: Metric | null) => {
      ...
      return (
        <div className="w-full max-w-full relative">
          ...
          <ChartContainer config={{ value: { label: 'Value', color } }} className="h-[220px] w-full">
            <BarChart
              data={chartData}
              layout="vertical"
              margin={{ left: 0 }}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <AggregationSettings
                    ...
                    onConditionTargetChange={(v) => updateProps({ conditionTarget: v })}
                    availableLabelTargets={availableLabels}
                    conditionOp={conditionOp}
                    onConditionOpChange={(v) => updateProps({ conditionOp: v })}
                    conditionValue={conditionValue}
                    onConditionValueChange={(v) => updateProps({ conditionValue: v })}
                    conditionValue2={conditionValue2}
                    onConditionValue2Change={(v) => updateProps({ conditionValue2: v })}
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <MultiSelect
                      options={availableLabels.map((label: string) => ({ value: label, label }))}
                      value={groupBy}
