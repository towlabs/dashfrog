...
const Flows = {
    ...
    history: (name: string, filters?: Filter[], pagination?: PaginationParams, dateRange?: DateRangeParams) => {
        const params = new URLSearchParams();
        if (pagination?.page !== undefined) {
            params.append('page', pagination.page.toString());
        }
        ...
        if (filters && filters.length > 0) {
            return FlowsAPI.post<PaginatedResponse<Flow>>(path, {
                data: { filters, from_date: dateRange?.from_date, to_date: dateRange?.to_date }
            });
        }
