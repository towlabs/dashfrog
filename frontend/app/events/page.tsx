...
import { CalendarIcon, ClockIcon, TagIcon, ChevronLeft, ChevronRight, ChevronDownIcon, X, Plus, Check, Shapes, AlertTriangle, Wrench, ClockArrowUp, ClockArrowDown, Clock } from 'lucide-react'
  ...
  for (let day = 0; day < 7; day++) {
    for (let hour = 0; hour < 24; hour++) {
      for (let minute = 0; minute < 60; minute += 10) {
        const date = new Date(startOfWeek.getTime() + (day * 24 * 60 * 60 * 1000) + (hour * 60 * 60 * 1000) + (minute * 60 * 1000))
        ...
        if (isFuture) {
          ...
          if ((dayOfWeek === 6 || dayOfWeek === 0) && hourOfDay >= 3 && hourOfDay <= 5) {
          ...
          const seedValue = timeKey.split('').reduce((a, b) => { return a + b.charCodeAt(0) }, 0)
          const random = (seedValue * 9301 + 49297) % 233280 / 233280 // Simple deterministic random

          // More incidents in the past
          if (random < 0.01) { // Higher chance for incidents
            status = 'incident'
          } else if ((dayOfWeek === 1 || dayOfWeek === 3) && hourOfDay >= 14 && hourOfDay <= 16 && random < 0.3) {
            status = 'incident' // Monday/Wednesday afternoon incidents
          } else if (dayOfWeek === 2 && hourOfDay >= 9 && hourOfDay <= 11 && random < 0.4) {
            status = 'incident' // Tuesday morning incident
          } else if ((dayOfWeek === 0 || dayOfWeek === 6) && hourOfDay >= 2 && hourOfDay <= 4 && random < 0.15) {
        ...
        data.push({
          ...
          minute,
          isFuture,
          events: status !== 'operational' && status !== 'future' ? [
            {
              title: status === 'incident' ? 'Service Incident' : 'Scheduled Maintenance',
...
export default function EventsPage() {
  ...
  const [eventType, setEventType] = useState<'incident' | 'maintenance'>('incident')
  const [eventLabels, setEventLabels] = useState<{id: string; key: string; value: string}[]>([])
  const [editingLabelId, setEditingLabelId] = useState<string | null>(null)
  const [searchTerm, setSearchTerm] = useState('')
  const [filters, setFilters] = useState<{id: string; column: string; operator: 'equals' | 'contains' | 'starts_with'; value: string}[]>([])
  ...
  const updateLabelValue = (labelId: string, newValue: string) => {
    if (!newValue.trim()) {
      ...
      setEventLabels(eventLabels.map(l =>
  ...
  const handleDayClick = (day: any) => {
    if (day.status === 'incident' || day.status === 'maintenance') {
      ...
      setEventEndTime(new Date(day.date.getTime() + 60 * 60 * 1000).toTimeString().slice(0, 5))
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => setWeekStart(getWeekStart(new Date()))}>This week</Button>
            <Button
              onClick={() => {
                const now = new Date()
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm text-muted-foreground mb-2">
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-1">
                    <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: '#addf7d' }}></div>
                    <span>Normal</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: '#e56458' }}></div>
                    <span>Incident</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: '#2783de' }}></div>
                ...
                {Array.from({ length: 24 }, (_, hour) => (
                  <div key={hour} className="flex gap-0.5">
                    {Array.from({ length: 6 }, (_, interval) => (
                      <div key={interval} className="flex-1" style={{ minWidth: '2px' }}>
                        {interval === 0 && (
              ...
              {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((dayName, dayIndex) => (
                <div key={dayName} className="flex items-center gap-2">
                  <div className="w-20 flex-shrink-0 text-xs text-muted-foreground font-medium">{dayName}</div>
                ...
                <DialogTitle className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${
                      statusColors[selectedDay.status as keyof typeof statusColors]
                    }`}
                  />
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Badge
                      className={`text-xs ${
                        selectedDay.status === 'operational' ? 'bg-green-100 text-green-800 border-green-200' :
                        selectedDay.status === 'degraded' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                        selectedDay.status === 'incident' ? 'bg-red-100 text-red-800 border-red-200' :
                        'bg-blue-100 text-blue-800 border-blue-200'
                      }`}
                      variant="outline"
                    >
                      {statusLabels[selectedDay.status as keyof typeof statusLabels]}
                    </Badge>
                  </div>
                </div>
                    ...
                    <div className="flex-1">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" className="justify-start text-left font-normal px-2 py-1 h-auto hover:bg-gray-100 rounded">
                            {eventType === 'incident' ? (
                              <div className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium" style={{ backgroundColor: '#f9dcd9', color: '#6d3531' }}>
                                <span className="w-2 h-2 rounded-full mr-1.5" style={{ backgroundColor: '#e56458' }} />
                                Incident
                              </div>
                            ) : (
                              <div className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium" style={{ backgroundColor: '#d8e4f1', color: '#264a72' }}>
                                <span className="w-2 h-2 rounded-full mr-1.5" style={{ backgroundColor: '#2783de' }} />
                                Maintenance
                              </div>
                            )}
                          </Button>
                        ...
                        <DropdownMenuContent align="start" className="w-[180px]">
                          <DropdownMenuItem onClick={() => setEventType('incident')} className="cursor-pointer">
                            <div className="w-full inline-flex items-center rounded-md px-2 py-1 text-xs font-medium" style={{ backgroundColor: '#f9dcd9', color: '#6d3531' }}>
                              <span className="w-2 h-2 rounded-full mr-1.5" style={{ backgroundColor: '#e56458' }} />
                              Incident
                            </div>
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setEventType('maintenance')} className="cursor-pointer">
                            <div className="w-full inline-flex items-center rounded-md px-2 py-1 text-xs font-medium" style={{ backgroundColor: '#d8e4f1', color: '#264a72' }}>
                              <span className="w-2 h-2 rounded-full mr-1.5" style={{ backgroundColor: '#2783de' }} />
                              Maintenance
                            </div>
                          </DropdownMenuItem>
                    ...
                    <div className="flex-1">
                      <div className="flex gap-2 flex-wrap items-center">
                        {eventLabels.map((label) => (
                          <div key={label.id} className="inline-flex items-center">
                            {editingLabelId === label.id ? (
                              <Popover open={true} onOpenChange={(open) => !open && updateLabelValue(label.id, label.value)}>
                                <PopoverTrigger asChild>
                                ...
                                <PopoverContent className="w-48 p-0" align="start">
                                  <Command>
                                    <CommandInput placeholder={`Search ${label.key} values...`} />
                                    <CommandList>
                                      <CommandEmpty>No options found.</CommandEmpty>
                                      <CommandGroup>
                                        {getLabelOptions(label.key).map((option) => (
                                          <CommandItem
                                            key={option}
                                            value={option}
                                            onSelect={() => updateLabelValue(label.id, option)}
                                          >
                                            <Check
                                              className={cn(
                                                "mr-2 h-4 w-4",
                                                label.value === option ? "opacity-100" : "opacity-0"
                                              )}
                                            />
                                            {option}
                                          </CommandItem>
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    setEventLabels(eventLabels.filter(l => l.id !== label.id))
                                  }}
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="start" className="w-48">
                            {['environment', 'severity', 'team', 'service', 'region'].map((labelKey) => (
                              <DropdownMenuItem
                                key={labelKey}
                                onClick={() => {
                                  if (!eventLabels.find(l => l.key === labelKey)) {
                                    const newLabel = {
                                      id: `label-${Date.now()}`,
                                      key: labelKey,
                                    ...
                                    setTimeout(() => setEditingLabelId(newLabel.id), 100)
                                  }
                                }}
