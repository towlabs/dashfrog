import { type NotebookData, type NotebookCreateInput, type NotebookUpdateInput } from './notebook-types'
...
class NotebookStorage {
  ...
  private serialize(notebook: NotebookData): string {
    const serialized = {
      ...notebook,
      timeWindow: notebook.timeWindow.type === 'absolute'
        ? {
            type: 'absolute',
            metadata: {
  ...
  private deserialize(data: string): NotebookData | null {
    try {
      ...
      return {
        ...parsed,
        timeWindow: parsed.timeWindow.type === 'absolute'
          ? {
              type: 'absolute',
              metadata: {
