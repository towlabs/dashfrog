...
import { Plus, X, Info, ChartLine, ChartSpline, ChartScatter, ArrowLeft, ChevronLeft } from "lucide-react"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const DEFAULT_METRICS: Metric[] = [
  ...
  },
  {
    ...
    description: "Status of a workflow, 1 for success, 0 for failure, and 2 for running",
    unit: "count",
    labels: ["workflow", "status"],
    metricType: "counter"
  }
...
const OPERATIONS: Operation[] = [
  { name: "average", label: "Average", description: "Calculate average value over time", type: "aggregation", applicableTo: ["statistic"] },
  { name: "p50", label: "Median", description: "50th percentile (median) over time", type: "aggregation", applicableTo: ["statistic"] },
  { name: "p90", label: "90th Percentile", description: "90th percentile over time", type: "aggregation", applicableTo: ["statistic"] },
  { name: "p95", label: "95th Percentile", description: "95th percentile over time", type: "aggregation", applicableTo: ["statistic"] },
  { name: "p99", label: "99th Percentile", description: "99th percentile over time", type: "aggregation", applicableTo: ["statistic"] },
]
...
export function MetricQueryBuilder({
  ...
  const [selectedMetric, setSelectedMetric] = React.useState<Metric | null>(initialMetric || null)
  const [filters, setFilters] = React.useState<Filter[]>(initialFilters || [])
  const [selectedOperation, setSelectedOperation] = React.useState<Operation | null>(initialOperation || null)
  ...
  const [metricSelectionStep, setMetricSelectionStep] = React.useState<'metrics' | 'operations'>('metrics')
  const [pendingStatistic, setPendingStatistic] = React.useState<Metric | null>(null)
  ...
  const availableOperations = (pendingStatistic || selectedMetric)
    ? OPERATIONS.filter(op => op.applicableTo.includes((pendingStatistic || selectedMetric)!.metricType))
    : []
    ...
    <div className={cn("flex flex-col gap-3", className)}>
      ...
      <div className="space-y-1">
        ...
        <Popover
          ...
          onOpenChange={(open) => {
            ...
            if (!open) {
              ...
              if (enableOperationSelector && selectedMetric?.metricType === 'statistic') {
        ...
        >
          ...
          <PopoverContent className="w-[320px] h-72 p-0 overflow-hidden" align="start">
              ...
              <div className="h-full flex flex-col">
                <Command className="flex-1">
                  ...
                  <CommandList className="max-h-full overflow-auto">
                    ...
                    <CommandGroup
                      heading={pendingStatistic ? `Apply function to ${pendingStatistic.label}` : 'Functions'}
                    >
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap gap-2">
            {filters.map((filter, index) => {
              ...
              if (filter.value.trim() === "" && editingFilterIndex !== index && !fadingFilters.has(index)) {
                return null
              }

              const isFading = fadingFilters.has(index)

              const currentFilter = editingFilterIndex === index && draftFilter ? draftFilter : filter
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            updateDraftFilter({ operator: value as FilterOperator })
                          }
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => updateDraftFilter({ value: e.target.value })}
            ...
            {draftFilter && editingFilterIndex !== null && editingFilterIndex >= filters.length && (
              <Popover
                open={true}
                onOpenChange={(open) => {
                  if (!open) {
              ...
              >
                <PopoverTrigger asChild>
                  ...
                  >
                    {draftFilter.label} {draftFilter.operator} {draftFilter.value || '""'}
                    <X
                      className="h-3 w-3 cursor-pointer hover:text-destructive"
                      onClick={(e) => {
                        e.stopPropagation()
