...
type Aggregation = 'average' | 'min' | 'max' | 'sum' | 'p50' | 'p90' | 'p95' | 'p99' | 'last' | 'share'
...
const AGG_GROUPS: Array<{
...
}> = [
  {
    heading: 'Basic',
    items: [
      { key: 'average', label: 'Average', description: 'Mean value over the time window' },
      { key: 'sum', label: 'Sum', description: 'Cumulative sum over the time window' },
  ...
  },
  {
    heading: 'Percentiles',
    items: [
      { key: 'p50', label: 'P50 (Median)', description: '50th percentile (median) of values' },
  ...
  },
  {
    heading: 'Latest',
    items: [
      { key: 'last', label: 'Last', description: 'Most recent value in the window' },
    ],
  },
  {
    heading: 'Derived',
    items: [
      { key: 'share', label: 'Share', description: 'Percent of rows matching a condition' },
    ],
  },
]
...
export function AggregationSettings({
  ...
  const selectedLabel = value ? getAggregationLabel(value as Aggregation) : 'Choose aggregation...'
      ...
      <div className="flex items-center gap-2">
        <Popover open={open} onOpenChange={setOpen} modal={true}>
          ...
          <PopoverContent className="w-[320px] h-72 p-0 overflow-hidden" align="start">
            <div className="h-full w-[640px] flex transition-transform duration-200 ease-out"
              style={{ transform: step === 'choose' ? 'translateX(0)' : 'translateX(-320px)' }}
            >
                    ...
                    {AGG_GROUPS.map((group) => (
                      <CommandGroup key={group.heading} heading={group.heading}>
                        {group.items.map((item) => (
                          <CommandItem key={item.key} value={item.key} onSelect={() => handleSelect(item.key)}>
              ...
              <div className="w-[320px] h-full p-3 space-y-3 overflow-auto">
                <div className="text-xs text-muted-foreground">Share condition</div>
                <div className="space-y-1">
                  <label className="text-xs text-muted-foreground font-medium">Target</label>
                  <Select value={conditionTarget ?? 'value'} onValueChange={(v) => onConditionTargetChange?.(v)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={conditionOp ?? 'eq'} onValueChange={(v) => onConditionOpChange?.(v)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <ChevronLeft className="h-3.5 w-3.5" />
                  ...
                  {
                    (() => {
                      const allowed = new Set(['eq','neq','contains','not_contains'])
                      const isValid = Boolean(conditionTarget && conditionValue && (conditionValue || '').trim() !== '' && conditionOp && allowed.has(String(conditionOp)))
                      return (
                        <Button
                          variant="default"
                          size="sm"
