...
export function SmartAutomationInput({ value, onChange, placeholder, className }: SmartAutomationInputProps) {
  ...
  const parseExpression = (expression: string): ParsedToken[] => {
    ...
    const patterns = [
      ...
      { type: 'operator', regex: /^(FAILED|SUCCESS|PENDING|STARTED|ENDED|>|<|>=|<=|==|!=|\+|\-|\*|\/)/i },
      { type: 'value', regex: /^"([^"]*)"/ },
      { type: 'value', regex: /^'([^']*)'/ },
  ...
  const getTokenColor = (type: string) => {
    switch (type) {
      ...
      case 'entity': return 'text-purple-600 font-medium'
      case 'property': return 'text-green-600'
      case 'operator': return 'text-orange-600 font-medium'
      case 'value': return 'text-red-600'
