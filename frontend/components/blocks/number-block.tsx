...
import { TimeWindowExclusionSelect, type ExclusionType } from '@/components/time-window-exclusion'
...
export const createNumberBlock = createReactBlockSpec(
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      const fetchNumberData = React.useCallback(async (
        metric: any,
        filters: any[],
        aggregation: string,
      ...
      React.useEffect(() => {
        if (selectedMetricValue) {
          fetchNumberData(selectedMetricValue, filtersValue, aggregation, timeWindow)
        }
      }, [
      ...
      const updateProps = React.useCallback((next: Partial<{
        label: string;
      ...
      const handleMetricChange = React.useCallback((metric: Metric | null) => {
      ...
      const handleOperationChange = React.useCallback((operation: Operation | null) => {
            ...
            <SheetContent className="w-[360px] sm:max-w-none p-0 flex h-full flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                ...
                <AggregationSettings
                  ...
                  onConditionTargetChange={(value) => updateProps({ conditionTarget: value })}
                  availableLabelTargets={getAvailableLabels()}
                  conditionOp={conditionOp}
                  onConditionOpChange={(value) => updateProps({ conditionOp: value })}
                  conditionValue={conditionValue}
                  onConditionValueChange={(value) => updateProps({ conditionValue: value })}
                  conditionValue2={conditionValue2}
                  onConditionValue2Change={(value) => updateProps({ conditionValue2: value })}
                />
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground font-medium">Exclude</label>
