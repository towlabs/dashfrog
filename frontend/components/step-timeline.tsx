...
const StepItem: React.FC<StepItemProps> = ({
  ...
  if (stepCreatedTime && stepStartedTime && stepEndedTime && totalDuration > 0) {
    ...
    if (showDelay) {
      delayStartPercent = ((stepCreatedTime - workflowStartTime) / totalDuration) * 100
    ...
    activeStartPercent = ((stepStartedTime - workflowStartTime) / totalDuration) * 100
      ...
      <div className="flex items-stretch hover:bg-muted/30 transition-colors">
        ...
        <div className="flex flex-col gap-1 py-2 flex-shrink-0 sticky left-0 top-0 bg-background z-10 w-80 border-r border-border">
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex items-start gap-2 flex-shrink-0" style={{ width: `${depth * 24 + 40}px` }}>
          ...
          {uniqueLabels.length > 0 && (
            <div className="flex gap-1 flex-wrap" style={{ paddingLeft: `${depth * 24 + 40}px` }}>
          ...
          {(step.description || step.status_reason) && (
            <div className="text-xs text-muted-foreground pr-2" style={{ paddingLeft: `${depth * 24 + 40}px` }}>
              {step.description && (
    ...
    return (
      <div className="p-8 border-2 border-dashed border-muted rounded-lg bg-muted/50">
        <div className="text-center text-muted-foreground">
          ...
          <div className="text-xs">This flow does not have any steps recorded</div>
        </div>
      </div>
    )
  }
