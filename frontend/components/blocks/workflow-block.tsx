...
import { CheckCircle2, XCircle, Clock, Plus, X } from 'lucide-react'
...
export const createWorkflowBlock = createReactBlockSpec(
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      const updateProps = React.useCallback((next: Partial<{ workflow: string; filters: string }>) => {
      ...
      const selectedWorkflow = AVAILABLE_WORKFLOWS.find(w => w.name === workflow)
            ...
            <Table>
              <TableHeader>
                <TableRow className="h-10">
                  ...
                  <TableHead className="text-right align-middle">Duration</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {demoSteps.map((step, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell className="text-right">{step.duration}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground font-medium">Workflow</label>
                    <Combobox
                      data={AVAILABLE_WORKFLOWS.map(w => ({ value: w.name, label: w.label }))}
                      type="workflow"
                      value={workflow}
                      onValueChange={(value) => {
