...
import { format, subMinutes, subHours, subDays, startOfWeek, endOfWeek } from 'date-fns'
import { cn } from '@/lib/utils'
import { Label } from '@/components/ui/label'
import { Separator } from '@/components/ui/separator'
import { DateTimePicker } from '@/components/ui/date-time-picker'
import type { TimeWindowConfig, RelativeTimeValue } from '@/lib/notebook-types'
...
const QUICK_RANGES: Array<{
  ...
  { label: 'Last 15 minutes', value: '15m', getValue: () => ({ start: subMinutes(new Date(), 15), end: new Date() }) },
  ...
  { label: 'Last 12 hours', value: '12h', getValue: () => ({ start: subHours(new Date(), 12), end: new Date() }) },
  { label: 'Last 24 hours', value: '24h', getValue: () => ({ start: subHours(new Date(), 24), end: new Date() }) },
  { label: 'Last 7 days', value: '7d', getValue: () => ({ start: subDays(new Date(), 7), end: new Date() }) },
  { label: 'Last 30 days', value: '30d', getValue: () => ({ start: subDays(new Date(), 30), end: new Date() }) },
  { label: 'This week', value: 'w', getValue: () => ({ start: startOfWeek(new Date(), { weekStartsOn: 0 }), end: endOfWeek(new Date(), { weekStartsOn: 0 }) }) },
]

export function TimeWindowSelector({ value, config, onChange, className }: TimeWindowSelectorProps) {
  const [open, setOpen] = React.useState(false)
  const [customStart, setCustomStart] = React.useState<Date | undefined>(value?.start)
  const [customEnd, setCustomEnd] = React.useState<Date | undefined>(value?.end)
  ...
  const currentWindow = value || { start: subHours(new Date(), 24), end: new Date() }
  ...
  const getDisplayLabel = () => {
    ...
    for (const range of QUICK_RANGES) {
      ...
      const diff = Math.abs(rangeValue.start.getTime() - currentWindow.start.getTime())
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className={cn('justify-between text-sm text-muted-foreground', className)}
        >
