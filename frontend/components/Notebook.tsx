...
import {type TimeWindow, TimeWindowSelector,} from "@/components/TimeWindowSelector";
import {Button} from "@/components/ui/button";
import {useNotebooks} from "@/src/contexts/notebooks";
import {Blocks, Notebooks} from "@/src/services/api/notebooks";
import {
    type NotebookData,
...
export default function Notebook({
    ...
    // Track lock confirmation dialog
    ...
    useEffect(() => {
        ...
        const loadBlocks = async () => {
            try {
                ...
                existingBlockIdsRef.current = new Set(response.data.map((b) => b.id));
     ...
     */
    const hasBlockChanged = useCallback(
        (newBlock: Block, oldBlock: Block | undefined): boolean => {
    ...
    const forceSave = useCallback(async () => {
        ...
        // Prevent saving if notebook is locked
        if (notebook.locked) {
            console.log("Cannot save: notebook is locked");
            return;
        }
        ...
        try {
            ...
            if (
                localTitle !== notebook.title ||
                localDescription !== notebook.description
            ) {
            ...
            const currentBlocks =
                pendingBlocksRef.current || currentBlocksRef.current;
            ...
            console.log(
                `[Force Save] Using ${pendingBlocksRef.current ? "pending" : "current"} blocks: ${currentBlocks.length} blocks`,
            );

            // Create a map of previous blocks for quick lookup
            const previousBlocksMap = new Map(previousBlocks.map((b) => [b.id, b]));

            // Identify operations needed
            const blocksToCreate = currentBlocks.filter(
            ...
            console.log(
                `Manual save (Ctrl/Cmd+S): ${blocksToCreate.length} created, ${blocksToUpdate.length} updated, ${blockIdsToDelete.length} deleted`,
            );
        } catch (err) {
    ...
    const handleBlocksChange = useCallback(
        (newBlocks: Block[]) => {
            ...
            // Prevent updates if notebook is locked
            if (notebook.locked) return;
            ...
            updateTimeoutRef.current = setTimeout(() => {
                const syncBlocks = async () => {
                    try {
                        ...
                        console.log(
                            `[Sync] Comparing ${newBlocks.length} blocks with ${previousBlocks.length} previous blocks`,
                        );
                        ...
                        console.log(
                            `[Sync] Operations: ${blocksToCreate.length} create, ${blocksToUpdate.length} update, ${blockIdsToDelete.length} delete`,
                        );
                        ...
                        if (blocksToUpdate.length > 0) {
                            ...
                            console.log(
                                `Batch updated ${blocksToUpdate.length} changed block(s)`,
                            );
                        }
                        ...
                        console.log(
                            `Sync complete: ${blocksToCreate.length} created, ${blocksToUpdate.length} updated, ${blockIdsToDelete.length} deleted`,
                        );
                    } catch (err) {
    ...
    const handleLockToggle = useCallback(async () => {
        ...
        try {
            // If unlocking, just unlock
            if (notebook.locked) {
                await Notebooks.unlock(notebook.id);
                // onUpdate will refresh the notebook from backend
                if (onUpdate) onUpdate();
                return;
            }

            // If locking, check for unsaved changes
            if (hasUnsavedChanges) {
                // Show confirmation dialog instead of native alert
                setShowLockConfirmation(true);
                return;
            }

            // Clear all pending timeouts to prevent parasitic updates
            if (updateTimeoutRef.current) {
                clearTimeout(updateTimeoutRef.current);
                updateTimeoutRef.current = null;
            }
            ...
            // Now lock the notebook
            await Notebooks.lock(notebook.id);
            // onUpdate will refresh the notebook from backend
            if (onUpdate) onUpdate();
        } catch (err) {
            console.error("Failed to toggle lock:", err);
        }
    }, [notebook, hasUnsavedChanges, onUpdate]);

    const handleLockConfirmation = useCallback(async () => {
        if (!notebook) return;
        ...
        try {
            ...
            console.error("Failed to lock notebook:", err);
        }
    }, [notebook, forceSave, onUpdate]);
        ...
        <div className="min-h-screen bg-background">
            ...
            {showLockConfirmation && (
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
                    <div className="bg-background border rounded-lg p-6 max-w-md mx-4 shadow-lg">
                        <h2 className="text-lg font-semibold mb-2">Unsaved Changes</h2>
                        <p className="text-sm text-muted-foreground mb-4">
                            You have unsaved changes. Do you want to save them before locking the notebook?
                        </p>
                        <div className="flex gap-3 justify-end">
                            <Button variant="outline" onClick={() => setShowLockConfirmation(false)}>
                                Cancel
                            </Button>
                            <Button onClick={handleLockConfirmation}>
                                Save and Lock
                            </Button>
                        </div>
                    </div>
            ...
            <div className="bg-background px-6 py-4">
                <div className="flex items-center justify-between">
                    {/* Left group: Lock/Unlock + Save indicator */}
                    {!isView && (
                        <div className="flex items-center gap-3">
                            ...
                            {hasUnsavedChanges && !showSavedIndicator && (
                                <div
                                    className="bg-yellow-500/10 border border-yellow-500/50 text-yellow-700 dark:text-yellow-400 px-3 py-1.5 rounded-md flex items-center gap-2 text-sm">
                            ...
                            {showSavedIndicator && (
                                <div
                                    className="bg-green-500/10 border border-green-500/50 text-green-700 dark:text-green-400 px-3 py-1.5 rounded-md flex items-center gap-2 text-sm">
