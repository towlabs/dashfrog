...
const AVAILABLE_WORKFLOWS = [
  { name: 'deployment_pipeline', label: 'Deployment Pipeline', labels: ['environment', 'branch', 'service'] },
  { name: 'data_processing', label: 'Data Processing', labels: ['dataset', 'region', 'priority'] },
  { name: 'ml_training', label: 'ML Training', labels: ['model', 'experiment', 'stage'] },
  { name: 'etl_job', label: 'ETL Job', labels: ['source', 'destination', 'schedule'] },
]
...
export const createWorkflowStatusMapBlock = createReactBlockSpec(
  ...
  },
  {
    render: ({ block, editor }) => {
      ...
      const parseWorkflows = (): Array<{ workflow: string; filters: Filter[] }> => {
        try {
      ...
      const [editingWorkflowIndex, setEditingWorkflowIndex] = React.useState<number | null>(null)
      const [filterOpen, setFilterOpen] = React.useState(false)
      const [editingFilterIndex, setEditingFilterIndex] = React.useState<number | null>(null)
      const [fadingFilters, setFadingFilters] = React.useState<Set<number>>(new Set())

      // Memoize updateWorkflows to prevent creating new function on every render
      const updateWorkflows = React.useCallback((next: Array<{ workflow: string; filters: Filter[] }>) => {
        // Only send what changed - BlockNote merges partial updates
        editor.updateBlock(block, { props: { workflows: JSON.stringify(next) } } as any)
      }, [editor, block.id])

      const addWorkflow = (workflowName: string) => {
        updateWorkflows([...workflows, { workflow: workflowName, filters: [] }])
      }
      ...
      const updateWorkflowFilters = (workflowIndex: number, filters: Filter[]) => {
      ...
      const addFilter = (workflowIndex: number, label: string) => {
        ...
        const newFilters = [...workflow.filters, { label, operator: '=' as FilterOperator, value: '' }]
        updateWorkflowFilters(workflowIndex, newFilters)
        setFilterOpen(false)
        setEditingFilterIndex(workflow.filters.length)
      }

      const updateFilter = (workflowIndex: number, filterIndex: number, updates: Partial<Filter>) => {
        const workflow = workflows[workflowIndex]
      ...
      const removeFilter = (workflowIndex: number, filterIndex: number) => {
        const workflow = workflows[workflowIndex]
        updateWorkflowFilters(workflowIndex, workflow.filters.filter((_, i) => i !== filterIndex))
      ...
      React.useEffect(() => {
        ...
        const timer = setTimeout(() => {
          const filtersToRemove = workflow.filters
            ...
            .filter(({ filter, index }) => filter.value.trim() === '' && editingFilterIndex !== index)

          if (filtersToRemove.length > 0) {
            setFadingFilters(new Set(filtersToRemove.map(f => f.index)))
            setTimeout(() => {
              updateWorkflowFilters(editingWorkflowIndex, workflow.filters.filter(f => f.value.trim() !== ''))
          ...
          <div className="rounded-md border overflow-auto">
            <div className="min-w-[800px]">
              ...
              <div className="flex border-b bg-muted/50">
                <div className="w-48 p-3 font-medium text-sm border-r">Workflow</div>
                <div className="flex-1 grid grid-cols-24 gap-0">
                  {Array.from({ length: 24 }, (_, i) => (
                    <div key={i} className="text-center text-xs p-1 text-muted-foreground">
                      {i % 4 === 0 ? `${i}h` : ''}
                    </div>
                  ))}
                </div>
              ...
              {workflows.length === 0 ? (
                ...
                demoData.map((data, idx) => {
                  const workflowInfo = AVAILABLE_WORKFLOWS.find(w => w.name === data.workflow)
                  return (
                    <div key={idx} className="flex items-center border-b last:border-b-0">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  {workflows.map((workflowItem, workflowIdx) => {
                    const workflowInfo = AVAILABLE_WORKFLOWS.find(w => w.name === workflowItem.workflow)
                    const isEditing = editingWorkflowIndex === workflowIdx

                    return (
                      <div key={workflowIdx} className="border rounded-md p-3 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-sm">{workflowInfo?.label || workflowItem.workflow}</span>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-6"
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {workflowItem.filters.map((filter, filterIdx) => {
                                if (filter.value.trim() === '' && (!isEditing || editingFilterIndex !== filterIdx) && !fadingFilters.has(filterIdx)) {
                                  return null
                                }
                                ...
                                return (
                                  <Popover
                                    key={filterIdx}
                                    open={isEditing && editingFilterIndex === filterIdx}
                                    onOpenChange={(open) => {
                                      if (open) {
                                      ...
                                      <div className="space-y-4">
                                        <div className="space-y-2">
                                          ...
                                          <Select
                                            ...
                                            onValueChange={(value) =>
                                              updateFilter(workflowIdx, filterIdx, { operator: value as FilterOperator })
                                            }
                                          >
                                        ...
                                        <div className="space-y-2">
                                          ...
                                          <Input
                                            value={filter.value}
                                            onChange={(e) => updateFilter(workflowIdx, filterIdx, { value: e.target.value })}
                              ...
                              <Popover
                                open={filterOpen && editingWorkflowIndex === workflowIdx}
                                onOpenChange={(open) => {
                                  setFilterOpen(open)
                                  if (open) setEditingWorkflowIndex(workflowIdx)
                                }}
                              >
                                <PopoverTrigger asChild>
                                  <Button
                                      ...
                                      <CommandGroup heading="Available Labels">
                                        {workflowInfo.labels?.map((label) => (
                                          <CommandItem
                                            key={label}
                                            value={label}
                                            onSelect={() => addFilter(workflowIdx, label)}
                                          >
                                            {label}
                                          </CommandItem>
                                        ))}
                          ...
                          <CommandGroup heading="Available Workflows">
                            {AVAILABLE_WORKFLOWS.map((w) => (
                              <CommandItem
                                ...
                                disabled={workflows.some(wf => wf.workflow === w.name)}
                              >
                                {w.label}
                              </CommandItem>
                            ))}
