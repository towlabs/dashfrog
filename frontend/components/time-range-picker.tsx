...
export function TimeRangePicker({ value, onChange }: TimeRangePickerProps) {
  ...
  const getDisplayText = () => {
    const quickRange = quickRanges.find(r => r.minutes.toString() === selectedRange)
    if (quickRange) {
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickRanges.map((range) => (
                <Button
                  key={range.minutes}
                  variant={selectedRange === range.minutes.toString() ? "default" : "outline"}
                  size="sm"
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={value.from ? format(value.from, "yyyy-MM-dd'T'HH:mm") : ''}
                    onChange={(e) => {
                      if (e.target.value) {
                ...
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={value.to ? format(value.to, "yyyy-MM-dd'T'HH:mm") : ''}
                    onChange={(e) => {
                      if (e.target.value) {
